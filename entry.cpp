...
void parseArgs(int argCount, char * argv[]) {
    ...
    for (int i =1; i < argCount; i++){
        ...
        else if (s.rfind("-t") == 0) useTestInput = true;
        else {
...
void runMultiple(int runs, std::vector<std::string> &lines, dayFunction fn) {
    ...
    for (int i = 0; i < runs; i++){
        dayResult.intResult1 = 0;
        dayResult.intResult2 = 0;
        auto t1 = high_resolution_clock::now();
        fn(lines, dayResult);
        auto t2 = high_resolution_clock::now();
...
void run(int argc, char * argv[]){
    ...
    std::string filePath;
    if (useTestInput) filePath = "./testInputs/day";
    else filePath = "./inputs/day";
    std::string fileName =  filePath + std::to_string(day) + ".txt";
    std::vector<std::string> input = getFileContent(fileName);
    auto fn = (*functptr[day - 1]);
